...
import { Search, UserPlus, Upload, Phone, MapPin, ReceiptText, X, CreditCard, FileText } from 'lucide-react';
...
export default function PartyKhataScreen() {
  ...
  const handleSavePayment = async () => {
    ...
    try {
      ...
      fetchParties(searchTerm); // Refresh left pane balances
    } catch (err) {
  ...
  return (
    <div className="max-w-[1600px] mx-auto pb-10 relative h-full flex flex-col">
      <div className="bg-white rounded-xl p-5 shadow-sm border border-slate-100 flex items-center justify-between mb-6 shrink-0">
        <div>
          <h2 className="text-xl font-bold text-slate-800 font-heading">Party Khata (Ledgers)</h2>
          <p className="text-sm text-slate-500 mt-0.5">Manage buyer/seller profiles, credit limits, and dynamic running balances</p>
        </div>
        <button 
          onClick={openDrawer}
      ...
      <div className="flex gap-6 flex-1 overflow-hidden">
        ...
        <div className="w-1/3 bg-white rounded-xl shadow-sm border border-slate-100 border border-slate-100 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar divide-y divide-slate-100">
            {parties.filter(p => Math.abs(p.outstanding) > 0 || searchTerm !== '').map(party => (
              <div 
                key={party.id} 
              ...
              >
                ...
                <div className="text-right">
                  <div className={`font-black text-sm ${party.outstanding > 0 ? 'text-rose-500' : party.outstanding < 0 ? 'text-emerald-500' : 'text-slate-400'}`}>
                    Rs {Math.abs(party.outstanding).toLocaleString()}
                  </div>
                  <p className="text-[10px] text-slate-400 font-bold uppercase">{party.outstanding > 0 ? 'Udhar' : party.outstanding < 0 ? 'Advance' : 'Cleared'}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="w-2/3 bg-white rounded-xl shadow-sm border border-slate-100 border border-slate-100 flex flex-col relative overflow-hidden">
          {selectedParty ? (
            <>
              {/* Profile Header */}
              <div className="p-6 border-b border-slate-100 bg-slate-50 flex items-start justify-between shrink-0">
                ...
                <div className="flex items-center gap-4">
                   <div className="text-right bg-white border border-slate-200 shadow-sm px-5 py-2.5 rounded-xl">
                     <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Total Outstanding</p>
                     <p className={`text-2xl font-black ${selectedParty.outstanding > 0 ? 'text-rose-500' : selectedParty.outstanding < 0 ? 'text-emerald-500' : 'text-slate-800'}`}>
                       Rs {Math.abs(selectedParty.outstanding).toLocaleString()}
                     </p>
                     <p className="text-[10px] font-bold mt-0.5 text-slate-500">
                       {selectedParty.outstanding > 0 ? '(Udhar)' : (selectedParty.outstanding < 0 ? '(Advance)' : 'Cleared')}
                     </p>
                   </div>
                ...
                <table className="w-full text-left text-sm">
                  <thead className="bg-white text-slate-500 font-bold border-b border-slate-200 uppercase tracking-wider text-[11px] sticky top-0 z-10 shadow-sm">
                    <tr>
                      ...
                      <th className="px-6 py-4 w-1/3">Description & Ref</th>
                      <th className="px-6 py-4 text-rose-500">Bill Amount</th>
                      <th className="px-6 py-4 text-emerald-500">Payment Received</th>
                      <th className="px-6 py-4 text-slate-800">Remaining Udhar</th>
                    ...
                    {ledger.length === 0 ? (
                      ...
                      ledger.map((tx, idx) => (
                        <tr key={idx} className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="px-6 py-4 font-bold text-rose-500">{tx.dr > 0 ? `Rs ${tx.dr.toLocaleString()}` : '-'}</td>
                          <td className="px-6 py-4 font-bold text-emerald-500">{tx.cr > 0 ? `Rs ${tx.cr.toLocaleString()}` : '-'}</td>
               ...
               <div className="bg-slate-800 p-5 rounded-xl shadow-md space-y-4">
                 ...
                 <div className="grid grid-cols-2 gap-4">
                   <div>
                     ...
                     <p className="text-[10px] text-slate-400 mt-1">Positive = Udhar (You Receive), Negative = Advance (You Pay)</p>
                   </div>
                   <div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center">
           <div className="bg-white rounded-2xl shadow-2xl w-[450px] overflow-hidden">
             ...
             <div className="p-5 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
                ...
                <button onClick={handleSavePayment} className="px-6 py-2 font-bold text-white bg-emerald-500 hover:bg-emerald-600 rounded-lg shadow-md">Post Payment</button>
             </div>
           </div>
        </div>
      )}
