...
export default function QuotationsScreen() {
  ...
  const shareWhatsApp = (q: any, its: any[]) => {
    const text = encodeURIComponent(`Quotation ${q.quotationId || q.quotationid}\nSupplier: ${q.supplierName || q.suppliername}\n${its.map(i=>`${i.itemName || i.itemname} - ${i.quantity} ${i.unit} @ Rs ${i.expectedRate || i.expectedrate}`).join('\n')}\nTotal: ${money(q.totalAmount ?? q.totalamount)}\nNotes: ${q.notes || '-'}`);
    window.open(`https://wa.me/${String(q.supplierPhone || q.supplierphone || '').replace(/\D/g,'')}?text=${text}`, '_blank');
  };
  ...
  return <div className="max-w-[1600px] mx-auto pb-10">
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-4">{rows.map(r=><div key={r.id} onClick={()=>openDetail(r.id)} className="bg-white rounded-2xl p-5 border shadow-sm hover:shadow-md cursor-pointer"><div className="flex justify-between"><div><p className="text-xs font-black text-slate-400">{r.quotationId || r.quotationid}</p><h3 className="font-black text-slate-900 mt-1">{r.supplierName || r.suppliername}</h3><p className="text-xs font-bold text-slate-500">{r.supplierPhone || r.supplierphone || '-'}</p></div><ClipboardList className="text-emerald-500"/></div><p className="mt-5 text-2xl font-black text-emerald-600">{money(r.totalAmount ?? r.totalamount)}</p><p className="text-xs font-bold text-slate-400 mt-1">{new Date(r.createdAt || r.createdat).toLocaleString()} • {r.status}</p></div>)}</div>
    ...
    {detail?.quotation && <div className="fixed inset-0 bg-slate-900/50 z-[999] flex items-center justify-center p-4"><div className="bg-white rounded-2xl max-w-4xl w-full p-6 shadow-2xl"><div className="flex justify-between"><h3 className="font-black text-xl">{detail.quotation.quotationId || detail.quotation.quotationid}</h3><button onClick={()=>setDetail(null)}><X/></button></div><p className="font-bold text-slate-500 mt-1">Supplier: {detail.quotation.supplierName || detail.quotation.suppliername} • {detail.quotation.supplierPhone || detail.quotation.supplierphone}</p><div className="mt-4 overflow-x-auto"><table className="w-full text-sm"><thead><tr className="bg-slate-100"><th className="p-3 text-left">Item</th><th>Qty</th><th>Rate</th><th>Total</th></tr></thead><tbody>{detail.items.map((i:any)=><tr key={i.id} className="border-b"><td className="p-3 font-bold">{i.itemName || i.itemname}<p className="text-xs text-slate-400">{i.notes}</p></td><td>{i.quantity} {i.unit}</td><td>{money(i.expectedRate || i.expectedrate)}</td><td className="font-black text-emerald-600">{money(i.total)}</td></tr>)}</tbody></table></div><div className="mt-4 flex justify-between bg-slate-900 text-white rounded-xl p-4"><span className="font-black">Total</span><span className="font-black text-emerald-300">{money(detail.quotation.totalAmount ?? detail.quotation.totalamount)}</span></div><div className="mt-5 flex flex-wrap gap-2"><button onClick={()=>window.print()} className="bg-slate-800 text-white rounded-xl px-4 py-2 font-black flex gap-2"><Printer size={16}/> Print</button><button onClick={()=>shareWhatsApp(detail.quotation, detail.items)} className="bg-emerald-600 text-white rounded-xl px-4 py-2 font-black flex gap-2"><MessageCircle size={16}/> WhatsApp</button><button onClick={()=>del(detail.quotation.id)} className="bg-rose-600 text-white rounded-xl px-4 py-2 font-black">Delete</button></div></div></div>}
  </div>;
}
